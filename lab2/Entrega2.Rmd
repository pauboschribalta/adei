...
pca_result <- PCA(df_numeric, quanti.sup = c(8), graph = FALSE, ind.sup=llmout)
...
```{r}
round(cbind(pca_result$var$coord[,1:2],pca_result$var$cos2[,1:2],pca_result$var$contrib[,1:2]),2)
...
```{r}
temp_cont = c("duration","campaign","emp.var.rate","cons.price.idx","cons.conf.idx","euribor3m","nr.employed","age_num")
temp_dis = var_dis[2:11]
pca_result2 = PCA(df[,c(temp_dis, var_res, temp_cont)], quali.sup = 1:11, graph = FALSE)
