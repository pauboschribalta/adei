...
When analyzing our specific problem with 'y' as the target variable in R, fitting a linear model could help to illustrate the linear relationship between the predictor variables and the expected value of 'y'. Nonetheless, it remains crucial to reflect upon the assumptions and limitations of linear regression and investigate alternative models which might more accurately model the probability of 'y' being a success.

```{r}
# Modelo inicial con dos variables numéricas y factores significativos
model <- glm(y ~ age_num + duration + job,
...
```{r}
model1 <- glm(y ~ age_num + duration + job + housing + loan + month,
             data = train_data, family = binomial)
summary(model1)
...
model2 <- glm(y ~ age_num * job + duration + housing + loan + month,
                          data = train_data, family = binomial)
...
model3 <- glm(y ~ age_num * job + # factor and covariate
              loan * housing + # couple of factors
              duration,
            data = train_data, family = binomial)
vif(model3)
...
```{r}
model4 <- glm(y ~ age_num * emp.var.rate + # factor and covariate
                  loan * housing + # couple of factors
                  duration,
                data = train_data, family = binomial)
vif(model4)
...
```{r}
model5 <- glm(y ~ age_num * emp.var.rate + # factor and covariate
                  loan * housing, # couple of factors
                data = train_data, family = binomial)
vif(model5)
marginalModelPlots(model5)
residualPlots(model5)
summary(model5)
```

```{r}
model6 <- glm(y ~ duration * emp.var.rate + # factor and covariate
                  loan * housing, # couple of factors
                data = train_data, family = binomial)
vif(model6)
marginalModelPlots(model6)
