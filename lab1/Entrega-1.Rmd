...
hist(df$age,
     ...
     xlim = c(min(df$age),
              ...
              ylab = "Frequència")
```
Agruparem en quatre possibles noves categories: Jove[0,25], Jove-Adult[26,45], Adult[46,65], Gran[+66].
...
```{r}
pie(prop.table(table(df$contact)),
        col = c("orange", "blue"),
...
hist(df$campaign,
     col = "blue",
              ylim = c(0, 4000),
              main = "Distribució de les campanyes anteriors", 
              xlab = "Número de campanyes", 
              ylab = "Frequència")
```
...
hist(pdays_aux,
     col = "blue",
              ylim = c(0, 30),
              main = "Distribució dels dies entre contactes", 
              xlab = "Número de dies", 
              ylab = "Frequència")
```
...
#Contarem només si hi ha hagut contacte previ o no
pie(prop.table(table(df$previous)), main = "Distribució campanya prèvia",
    col = c("red", "green"))
```
...
```{r}
table(df$cons.price.idx)
```

#### Consumer confidence index

```{r}
table(df$cons.conf.idx)
```
...
treshold <- quantile(df$age, 0.75)*3
abline(h = treshold, col = "red", lty = "dashed")
```

```{r}
out_dur <- which(df$age > treshold)
length(out_dur)
```

```{r}
boxplot(df$duration, col = "blue")
